...
async def start(message: Message, state: FSMContext):
    await message.answer(MessageTemplates.WELCOME)
...
async def set_profile(message: Message, state: FSMContext):
    await message.answer(MessageTemplates.WEIGHT_REQUEST)
...
async def set_weight(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(MessageTemplates.WEIGHT_ERROR)
        return
    await state.update_data({'weight': weight})

    await message.answer(MessageTemplates.HEIGHT_REQUEST)
...
async def set_height(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(MessageTemplates.HEIGHT_ERROR)
        return
    await state.update_data({'height': height})

    await message.answer(MessageTemplates.AGE_REQUEST)
...
async def set_age(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(MessageTemplates.AGE_ERROR)
        return
    await state.update_data({'age': age})

    await message.answer(MessageTemplates.ACTIVITY_REQUEST)
...
async def set_activity_rate(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(MessageTemplates.ACTIVITY_ERROR)
        return
    await state.update_data({'activity_rate': activity_rate})

    await message.answer(MessageTemplates.CITY_REQUEST)
...
async def set_city(message: Message, state: FSMContext):
    ...
    await message.answer(
        MessageTemplates.TARGET_CALORIES_CALCULATED.format(calories=calories),
...
async def request_calories(message: Message, state: FSMContext):
    ...
    await message.answer(MessageTemplates.TARGET_CALORIES_REQUEST)
...
async def set_calories(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(MessageTemplates.TARGET_CALORIES_ERROR)
        return
    await state.update_data({'target_calories': calories, 'target_set_by_user': True})

    state_data = await state.get_data()
    await message.answer(generate_profile_summary(state_data))
